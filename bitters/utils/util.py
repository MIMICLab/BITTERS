...
@rank_zero_only
def save_eval_results(log_dir, originals, generated, caption_references, caption_candidates):
    f_ref =  open(os.path.join(log_dir,'caption_ref.txt'), 'w') 
    for caption in caption_references:       
        print(caption,file=f_ref)
    f_ref.close()
    f_cand = open(os.path.join(log_dir,'caption_cand.txt'), 'w')
    for caption in caption_candidates:       
        print(caption,file=f_cand)
    f_cand.close()
    with open(os.path.join(log_dir,'originals.npy'), 'wb') as f:
        np.save(f, np.array(originals))        
    with open(os.path.join(log_dir,'generated.npy'), 'wb') as f:
        np.save(f, np.array(generated))
